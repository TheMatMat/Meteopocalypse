...
namespace PrintLib
{
    ...
    public class Printer
    {
        ...
        [DllImport("PrintLibDll")]
        private static extern int _SetPrinterSettings(int Orientation, int PaperSize, int DPI, int ColorMode, int Copies);
        ...
        public int SetPrinterSettings(Orientation orientation, PaperFormat paperFormat, int DPI = 0, ColorMode colorMode = ColorMode.Default, int copies = 0)
        {
            // note: a value of 0 means that the setting won't be changed and the default value will be used instead
            return _SetPrinterSettings((int)orientation, (int)paperFormat, DPI, (int)colorMode, (int)copies);
        }
